...
(function () {
  ...
  function detectSLCodes(text) {
    var raw = text.match(/(?:SL|PR)\d{10,}/g);
    if (!raw) return [];
  ...
  function detectUsers(text) {
    var raw = text.match(/(?:mp-|srv-|usr_|dba-|app-)[a-zA-Z0-9_\-]+/g);
    if (!raw) return [];
    var seen = {};
  ...
  function detectDBObjects(text) {
    ...
    var schemaRaw = text.match(/[A-Za-z_][A-Za-z0-9_]*\.[A-Za-z_][A-Za-z0-9_]{3,}/g);
    if (schemaRaw) {
    ...
    var spRaw = text.match(/\b(?:sp_|usp_|SP_|USP_)[A-Za-z0-9_]{3,}/g);
    if (spRaw) spRaw.forEach(function (v) { matches.push(v); });

    // Pattern 3: Common DB prefixes (HN_, VW_, ZVW_, FN_, TBL_, V_, T_)
    var prefixRaw = text.match(/\b(?:HN_|VW_|ZVW_|FN_|TBL_|V_|T_)[A-Za-z0-9_]{3,}/g);
    if (prefixRaw) prefixRaw.forEach(function (v) { matches.push(v); });

    // Pattern 4: Contextual - word after keywords
    var contextRaw = text.match(/(?:tabla|vista|procedimiento|store\s*procedure|view|trigger|function|función|índice|index)\s*[:\-]?\s*([A-Za-z_][A-Za-z0-9_.]{3,})/gi);
    if (contextRaw) {
    ...
    var upperRaw = text.match(/\b[A-Z][A-Z0-9]*(?:_[A-Z0-9]+){2,}\b/g);
    if (upperRaw) {
  ...
  function renderDetections(detections, options) {
    ...
    var hasContent = (detections.slCodes.length && showLabels) ||
