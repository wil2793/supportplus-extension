(function () {
  ...
  async function loadTeamPanel() {
    ...
    try {
      ...
      profiles.forEach(function(p) {
        fetch(SP_SEARCH_API + "?resolutionGroupId=19&responsibleName=" + encodeURIComponent(p.profileFullName), {
          headers: { accept: "application/json", authorization: "Bearer " + spToken },
  ...
  function refreshTeamColumn(profileId) {
    ...
    if (!name) return;
    fetch(SP_SEARCH_API + "?resolutionGroupId=19&responsibleName=" + encodeURIComponent(name), {
      headers: { accept: "application/json", authorization: "Bearer " + spToken },
    }).then(function(r) { return r.json(); }).then(function(json) {
      var tickets = ((json.data || json).content || []).filter(function(t) {
        return t.ticketStatusName === "Asignado" || t.ticketStatusName === "En atención";
