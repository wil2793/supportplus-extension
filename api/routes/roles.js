...
router.get("/", asyncHandler(async (req, res) => {
  const rows = await query(`SELECT IdcatRol, Nombre, Descripcion FROM ESP_cat_Rol WHERE Activo = 1 ORDER BY Nombre`);
  success(res, rows);
}));
...
router.get("/:id", asyncHandler(async (req, res) => {
  const row = await queryOne(
    `SELECT IdcatRol, Nombre, Descripcion FROM ESP_cat_Rol WHERE IdcatRol = @id AND Activo = 1`,
...
router.post("/", asyncHandler(async (req, res) => {
  ...
  const inserted = await insertOne(
    `INSERT INTO ESP_cat_Rol (Nombre, Descripcion, UsuarioAlta)
...
router.delete("/:id", asyncHandler(async (req, res) => {
  ...
  await execute(
    `UPDATE ESP_cat_Rol SET Activo = 0, UsuarioBaja = @usuario, FechaBaja = GETDATE() WHERE IdcatRol = @id`,
    {
      id: { type: sql.Int, value: req.params.id },
...
router.post("/asignar", asyncHandler(async (req, res) => {
  ...
  const exists = await queryOne(
    "SELECT 1 FROM ESP_rel_UsuarioRol WHERE FK_IdUsuario = @u AND FK_IdcatRol = @r AND Activo = 1",
    { u: { type: sql.Int, value: fkIdUsuario }, r: { type: sql.Int, value: fkIdcatRol } }
  );
  if (exists) return fail(res, "Rol ya asignado a este usuario");

  const inserted = await insertOne(
    `INSERT INTO ESP_rel_UsuarioRol (FK_IdUsuario, FK_IdcatRol, UsuarioAlta)
...
router.delete("/asignar/:id", asyncHandler(async (req, res) => {
  const { usuarioBaja } = req.body;
  await execute(
    `UPDATE ESP_rel_UsuarioRol SET Activo = 0, UsuarioBaja = @usuario, FechaBaja = GETDATE() WHERE IdrelUsuarioRol = @id`,
    {
      id: { type: sql.Int, value: req.params.id },
      usuario: { type: sql.VarChar(50), value: usuarioBaja || "SISTEMA" }
    }
