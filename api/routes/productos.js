...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const members = await query(
      "SELECT FK_IdUsuario FROM MSP_rel_ProductoUsuario WHERE FK_IdcatProducto = @p AND Activo = 1",
      { p: fkIdProducto },
    );
    if (members.length > 0) {
      const allHaveLog = await query(
        `SELECT pu.FK_IdUsuario,
              (SELECT COUNT(*) FROM MSP_LogProducto lp WHERE lp.FK_IdcatProducto = @p AND lp.FK_IdUsuario = pu.FK_IdUsuario AND lp.Activo = 1) AS LogCount
       FROM MSP_rel_ProductoUsuario pu
      ...
      if (everyoneHasOne) {
        ...
        for (const m of allHaveLog) {
          await execute(
            `UPDATE TOP (1) MSP_LogProducto SET Activo = 0, UsuarioModificacion = 'SISTEMA_RESET', FechaModificacion = GETDATE()
           WHERE FK_IdcatProducto = @p AND FK_IdUsuario = @u AND Activo = 1
           AND IdLogProducto = (SELECT TOP 1 IdLogProducto FROM MSP_LogProducto WHERE FK_IdcatProducto = @p AND FK_IdUsuario = @u AND Activo = 1 ORDER BY FechaAlta ASC)`,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const exists = await queryOne(
      "SELECT 1 FROM MSP_rel_ProductoUsuario WHERE FK_IdcatProducto = @p AND FK_IdUsuario = @u AND Activo = 1",
      { p: fkIdProducto, u: fkIdUsuario },
    );
    if (exists) return success(res, { message: "Ya asignado" });

    await execute(
      `INSERT INTO MSP_rel_ProductoUsuario (FK_IdcatProducto, FK_IdUsuario, UsuarioAlta, FechaAlta, Activo)
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    await execute(
      `UPDATE MSP_rel_ProductoUsuario SET Activo = 0, UsuarioBaja = @mod, FechaBaja = GETDATE()
     WHERE FK_IdcatProducto = @p AND FK_IdUsuario = @u AND Activo = 1`,
      {
        p: fkIdProducto,
        u: fkIdUsuario,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const rows = await query(
      `INSERT INTO MSP_cat_Producto (Nombre, Cantidad, UsuarioAlta, FechaAlta, Activo)
