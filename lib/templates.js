...
(function () {
  ...
  // Use window.esc from components.js (loaded before this file per manifest order)
  function esc(str) {
  ...
  function ticketCard(ticket, options) {
    ...
    const cursor = opts.draggable ? "cursor:grab;" : "";
    const borderColor = opts.borderColor || "#eee";
    const codeColor = opts.codeColor || "#1976D2";
    const t = ticket;

    const parts = [];
    parts.push('<div ' + draggable + 'data-ticket-id="' + t.id + '" class="sp-mgr-ticket" style="display:block;padding:3px 5px;margin:2px 0;border-radius:4px;background:#fff;border:1px solid ' + borderColor + ';font-size:9px;line-height:1.3;' + cursor + '">');
    parts.push('<div style="font-weight:600;color:' + codeColor + ';">' + esc(t.uniqueCode || "") + '</div>');
    parts.push('<div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#555;">' + esc((t.subject || "").substring(0, 25)) + '</div>');

    if (opts.showStatus) {
      const statusColor = getStatusColor(t.ticketStatusName);
      const nameToShow = opts.showResponsible ? (t.responsibleName || "") : (t.requesterName || "");
      parts.push('<div style="display:flex;justify-content:space-between;">');
      parts.push('<span style="color:' + statusColor + ';font-weight:600;font-size:8px;">' + esc(t.ticketStatusName || "") + '</span>');
      parts.push('<span style="color:#888;font-size:8px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:60px;" title="' + esc(nameToShow) + '">' + esc(nameToShow.split(" ")[0]) + '</span>');
      parts.push('</div>');
    } else {
      const person = opts.showResponsible ? (t.responsibleName || "") : (t.requesterName || "");
      parts.push('<div style="color:#888;font-size:8px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' + esc(person) + '">' + esc(person.split(" ")[0]) + '</div>');
    }
  ...
  function columnHeader(title, bgColor, countClass) {
    const cls = countClass || "sp-mgr-pcount";
  ...
  function pendingTicketCard(pt, withinHours) {
    ...
    const parts = [];
    parts.push('<div class="sp-mgr-ticket sp-pending-ticket" data-ticket-id="' + pt.ticketId + '" style="display:block;padding:3px 5px;margin:2px 0;border-radius:4px;background:#fff;border:1px solid #FF8F00;font-size:9px;line-height:1.3;' + cursor + '">');
    parts.push('<div style="font-weight:600;color:#E65100;">' + esc(pt.ticket) + '</div>');
    if (!withinHours) {
      parts.push('<div style="color:#888;font-size:8px;">🔒 Fuera de horario</div>');
    }
    parts.push('</div>');
    return parts.join("");
  }
