...
(function () {
  ...
  var SP_API = window.SP_CONFIG.SP_API;
  var SP_SEARCH_API = window.SP_CONFIG.SP_SEARCH_API;

  // ─── Cache ────────────────────────────────────────────────
  var _mondayTokenCache = "";
  var _mondayUsersCache = null; // Promise<Object> email->id
  var _mondayBoardsCache = {}; // key -> boardId
  ...
  async function spFetch(endpoint, options) {
    var url = endpoint.startsWith("http") ? endpoint : SP_API + "/" + endpoint;
  ...
  async function spSearchTickets(params) {
    var query = Object.keys(params)
      ...
      .join("&");
    var url = SP_SEARCH_API + "?" + query;
    var json = await spFetch(url);
  ...
  async function getMondayWorkspaceId(groupId) {
    if (_mondayWorkspaceCache) return _mondayWorkspaceCache;
    var stored = await SP_Storage.get("groupMondayConfig");
    var config = stored || {};
    var gId = groupId || Object.keys(config)[0] || "";
    if (gId && config[gId]) {
      _mondayWorkspaceCache = config[gId].workspaceId || "";
    }
    return _mondayWorkspaceCache;
  }
  ...
  async function getMondayTicketBoards(token, workspaceId) {
    var wsId = workspaceId || (await getMondayWorkspaceId());
    if (!wsId) return [];
    var data = await mondayQuery(token, "{ boards(workspace_ids: [" + wsId + "], limit: 50) { id name } }", {});
  ...
  function getMondayUsers(token) {
    if (!_mondayUsersCache) {
      _mondayUsersCache = mondayQuery(token, "{ users(limit:500) { id email } }", {})
        .then(function (data) {
          var map = {};
        ...
        .catch(function () {
          _mondayUsersCache = null;
          return {};
        });
    }
    return _mondayUsersCache;
  }
  ...
  async function getMondayBoardForMonth(year, month, groupId) {
    var mondayConfig = await getMondayConfigForGroup(groupId);
    if (!mondayConfig || !mondayConfig.etiqueta) return null;

    var key = groupId + "-" + year + "-" + String(month + 1).padStart(2, "0");
  ...
  async function checkTicketInMonday(token, uniqueCode) {
    ...
    for (var i = 0; i < boards.length; i++) {
      try {
        ...
        var items = (res.items_page_by_column_values && res.items_page_by_column_values.items) || [];
