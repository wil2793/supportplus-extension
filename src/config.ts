// ============================================================
// SRC/CONFIG.TS - Centralized constants and configuration
// ============================================================

import { SP_ENV } from "./env";
import type { SpConfig, GroupInfo } from "./types";
...
export type SpStatusId = (typeof SP_STATUSES)[keyof typeof SP_STATUSES];
...
// SP status name (lowercase) → Monday column index
...
/** Map a SP status name to the corresponding Monday column index. */
export function mapStatusToMonday(statusName: string): number {
  const s = (statusName ?? "").toLowerCase();
  return STATUS_MAP[s] !== undefined ? STATUS_MAP[s] : 5;
}
...
export const SP_CONFIG: SpConfig = {
  API_URL: SP_ENV.API_URL ?? "https://back-extension-sp.macropay.mx/api",
