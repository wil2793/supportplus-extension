...
import { showErrorToast, showLoadingToast } from "../components";
...
export async function showHistoryModal(ticketId: number | string): Promise<void> {
  ...
  try {
    ...
    const cardsHtml = logs.length
      ? logs.map((log) => {
          const ai = ACTION_MAP[log.action?.name as string] ?? { label: log.action?.name ?? "Acción", color: "#757575" };
