...
export function showConfigModal(ctx: ConfigModalContext): void {
  ...
  chrome.storage.local.get(["mondayToken", "teamArea"], (stored: AnyObj) => {
    ...
    setupTabs();
    // Member list
    const membersDiv = document.getElementById("sp-cfg-members") as HTMLElement;
    ...
    (
      ...
      if (mondayTokenInput && mondayTokenInput !== "••••••••") {
        ...
        chrome.storage.local.get(["usersMap", "userEmail"], (nd: AnyObj) => {
          const email: string = (
      ...
      chrome.storage.local.get(["usersMap", "userEmail"], (nd: AnyObj) => {
        const email: string = ((nd["userEmail"] as string) ?? "").toLowerCase();
        const user: AnyObj = ((nd["usersMap"] as AnyObj) ?? {})[email];
...
function loadMembersForArea(
  ...
  void fetch(
    ...
    .then((json: Record<string, any>) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const profiles: Array<Record<string, any>> = json["data"] ?? json;
